...
struct whisper_state {
    ...
    int32_t encoded_seek = -1;   // mel offset the encoder output in kv_cross was computed at, -1 = none
...
static bool whisper_encode_internal(
    ...
    wstate.n_encode++;
    wstate.encoded_seek = mel_offset;
...
int whisper_full_with_state(
    ...
    // new audio: whatever the encoder computed for the previous call is stale
    ...
    while (true) {
        ...
        // encode audio features starting at offset seek, unless language detection just did exactly that
        // (same offset, same audio context): its output is still in kv_cross
        if (state->encoded_seek != seek && !whisper_encode_internal(*ctx, *state, seek, params.n_threads)) {
            fprintf(stderr, "%s: failed to encode\n", __func__);
            return -6;
        }
